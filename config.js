...
window.MLT_CONFIG = {
  ...
  /**
   * Ключ Faceit Data API (https://developers.faceit.com/) — нужен для
   * кнопки «Обновить Elo с Faceit» в профиле. Работает без сервера: сайт
   * стучится в Faceit прямо из браузера пользователя, поэтому проект
   * может оставаться на бесплатном плане Firebase (Spark), без Cloud
   * Functions и без GitHub Actions.
   *
   * Из-за этого ключ виден всем в открытом коде сайта — как и apiKey
   * Firebase выше. Это НЕ пароль и не даёт доступа к чьим-либо
   * аккаунтам — только к публичной статистике игроков, так что для
   * такого сайта это приемлемый компромисс. Если для вас это критично —
   * единственная безопасная альтернатива — прежняя схема с GitHub
   * Actions (см. FACEIT_SETUP.md, раздел «Альтернатива»).
   */
  faceit: {
    apiKey: "2dd4e1b0-230b-490a-9512-ebb74e96ad09",
  },
